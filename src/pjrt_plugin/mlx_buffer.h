...
public:
    ...
    // Buffer metadata
    MlxDevice* device() const { return device_; }
    int dtype() const { return pjrt_dtype_; }
    const std::vector<int64_t>& dimensions() const { return dims_; }
    int64_t element_count() const;
    size_t byte_size() const;

    // Data access
    mlx::core::array& array() { return array_; }
    const mlx::core::array& array() const { return array_; }
