...
PJRT_Error* MPS_DeviceDescription_Id(PJRT_DeviceDescription_Id_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_DeviceDescription_Id called\n");
...
PJRT_Error* MPS_Device_GetDescription(PJRT_Device_GetDescription_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_GetDescription called, device=%p\n", (void*)args->device);
    if (args->device) {
        args->device_description = args->device->description;
        JAXPLUGIN_LOG_DEBUG(" Returning description=%p\n", (void*)args->device_description);
    } else {
...
PJRT_Error* MPS_Device_IsAddressable(PJRT_Device_IsAddressable_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_IsAddressable called, device=%p\n", (void*)args->device);
    args->is_addressable = true;
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_IsAddressable returning\n");
    return nullptr;
}

PJRT_Error* MPS_Device_LocalHardwareId(PJRT_Device_LocalHardwareId_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_LocalHardwareId called, device=%p\n", (void*)args->device);
    args->local_hardware_id =
        args->device && args->device->device ? args->device->device->local_hardware_id() : 0;
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_LocalHardwareId returning %d\n", args->local_hardware_id);
    return nullptr;
}

PJRT_Error* MPS_Device_AddressableMemories(PJRT_Device_AddressableMemories_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_AddressableMemories called\n");
...
PJRT_Error* MPS_Device_DefaultMemory(PJRT_Device_DefaultMemory_Args* args) {
    JAXPLUGIN_LOG_DEBUG(" PJRT_Device_DefaultMemory called, device=%p\n", (void*)args->device);
    if (args->device) {
        args->memory = args->device->default_memory;
        JAXPLUGIN_LOG_DEBUG(" Returning memory=%p\n", (void*)args->memory);
    } else {
        args->memory = nullptr;
    }
    return nullptr;
